...
function getFreshnessClass(dateString) {
  ...
  function sortByKey(key) {
    ...
    updateSortIndicator();
    if (typeof renderCurrencyList === 'function') renderCurrencyList(latestLoadedRows);
  }
  ...
  function drawRangeChart(seriesArg, pair) {
    ...
    if (!seriesList.length || !seriesList[0].data || seriesList[0].data.length === 0) {
      ...
      ctx.fillText('Seçilen aralıkta grafik verisi bulunamadı.', 16, 30);
      return;
    }
  ...
  async function loadRangeChart(startDateValue, endDateValue, pair) {
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        const rows = (typeof filterVisibleRows === 'function') ? filterVisibleRows(parseAllProviders(fullSnapshot)) : parseAllProviders(fullSnapshot);
  ...
  export async function loadLatestData() {
    ...
    try {
      ...
      let rows = [];
      try {
        rows = (typeof filterVisibleRows === 'function') ? filterVisibleRows(parseAllProviders(latest)) : parseAllProviders(latest);
      } catch (e) {
      ...
      if (typeof updateSummaryCards === 'function') updateSummaryCards(rows);
      if (typeof renderCurrencyList === 'function') renderCurrencyList(rows);
  ...
  export async function loadMonthlyData(monthKey) {
    ...
    try {
      ...
      if (!entries.length) {
        ...
        if (typeof renderCurrencyList === 'function') renderCurrencyList([]);
        return;
      }

      const latestEntry = entries[entries.length - 1];
      const rows = (typeof filterVisibleRows === 'function') ? filterVisibleRows(parseAllProviders(latestEntry)) : parseAllProviders(latestEntry);
  ...
  function wireRangeLoader() {
    ...
    button.addEventListener('click', () => {
      ...
      if (typeof loadChartFromInputs === 'function') void loadChartFromInputs();
    });
  }
  ...
  async function init() {
    ...
    try {
      ...
      const chartPromise = (async () => {
        ...
        if (typeof loadChartFromInputsWithRetry === 'function') await loadChartFromInputsWithRetry(3);
      })();
